# TensorFlow 1.5 supporting Python for AVX instruction-void CPU support
TensorFlow use on AVX instruction void CPU could still be as good use as on otherwise nowadays   
...
Only the point of problem is now; it's to be utilized with Python version 3.6 or so only, which is commonly difficult to build due to compatiblility flaws or version discrepancies between old and new S/W package leading to build inaccuracies/incorrectness, and due to it's been in EOL state (on its official page):   
...
So here and now, this page would try to fix and revive the python 3.6 project and have the python correctly built up to be made use for old TensorFlow   
It will need, as dependency, a certain openSSL version   
...
12. Enter to the Python 3.6.15 source directory  and do the build CL    
    The CL will compile it against the newly made openSSL static library to installing in /usr/bin, and it's set to skip some needless PGO build tests:   
...
13. **Please note**: As `make` installs to `/usr/bin`, this wil replace main/system-wide `python` link to link to this 3.6.15
    So as most installation is link to the newer/newest one, linking back to it:
...
15. Have a python 3.6 environment, eg. namedly `TensorFlow1.5` in home directory or etc
    $ python3.6 -m venv ~/TensorFlow1.5

16. Enter it and Install `pip`
    . ~/TensorFlow1.5/activate   
    curl https://bootstrap.pypa.io/pip/3.6/get-pip.py  | python   

17. Ensure TensorFlow dependencies
...
20. Upon success satisfying every rerquirement, one of the most popular AI tool;
    Tandem of Python and TensorFlow, should've been afforded      
